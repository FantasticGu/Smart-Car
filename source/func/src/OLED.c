...
#define OLED_SDA_PIN  PTC17
#define OLED_RST_PIN  PTC18
#define OLED_DC_PIN   PTC19
...
void OLEDxianshi(int line_use)
{
  ...
  for(int j=0;j<64;j++)
  {
    for(int i=0;i<128;i++)image[j][i]=0;
  }
  for(int j=0;j<8;j++)
  {
    for(int i=0;i<128;i++)bmp[j][i]=0;
  }
  ...
  for(int j=0;j<8;j++)
  {
    for(int i=0;i<128;i++)
    {
      for(int k=0;k<8;k++)bmp[j][i]=bmp[j][i]+((image[j*8+k][i])<<(k));
    }   
  }
